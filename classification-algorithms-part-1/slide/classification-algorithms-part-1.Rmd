...
特征选择主要解决的问题的用来进行划分特征空间的特征的顺序以及特征自身的划分依据等。根据 s 数据集可知，是否有客户光顾高尔夫俱乐部收到天气，气温，湿度和有无风四个变量的制约。其中，天气和有无风为离散型变量，气温和湿度为连续型变量。那么在构建决策树时，会有两个基本问题：
...
.pull-right[
则高尔夫俱乐部光顾数据集关于特征“天气”的 Gini 指数 $\textrm{Gini}\left(D, A\right)$：
