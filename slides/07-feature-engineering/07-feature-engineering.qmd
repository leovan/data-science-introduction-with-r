...
1. 特征工程是利用对数据的业务理解构建适合特定机器学习算法的特征的过程 ^[Wikipedia, “Feature engineering.” <https://en.wikipedia.org/wiki/Feature_engineering>]。
2. 特征工程是将原始数据转换成特征的过程。这些特征能够更好的将根本问题表达表述成预测模型，同时利用不可直接观测的数据提高模型准确性 ^[Brownlee, “Discover feature engineering, how to engineer features and how to get good at it.” <http://machinelearningmastery.com/discover-feature-engineering-how-to-engineer-features-and-how-to-get-good-at-it/>]。
3. 特征工程就是人工设计模型的输入变量 $x$ 的过程 ^[T. Malisiewicz, “What is feature engineering?.” <https://www.quora.com/What-is-feature-engineering>]。

## 特征工程

从数据挖掘过程的角度，对“传统的”特征工程给出如下定义。特征工程就是指从原始数据（即通过现实生活中的实际业务发生产生的数据）加工得到最终用于特定的挖掘算法的输入变量的过程。此处之所以强调是“传统的”特征工程主要是用于区分其和近期基于深度学习等方法的自动特征生成。据此将特征工程划分为如下几个阶段：
...
::: {.column width="50%"}
![](images/xkcd-machine-learning.png){fig-align="center" width=70%}
:::
::::

::: aside
图片来源：<https://xkcd.com/1838/>
:::
...
对于数据缺失的情况，Rubin ^[Rubin, Donald B. "Inference and missing data." _Biometrika_ 63.3 (1976): 581-592.] 从缺失机制的角度分为 3 类：完全随机缺失（missing completely at random，MCAR），随机缺失（missing at random）和非随机缺失（missing not at random，MNAR）。在 Missing Data ^[Allison, Paul D. _Missing data_. Vol. 136. Sage publications, 2001.] 中定义有，对于一个数据集，变量 $Y$ 存在数据缺失，如果 $Y$ 的缺失不依赖于 $Y$ 和数据集中其他的变量，称之为 MCAR。如果在控制其他变量的前提下，变量 $Y$ 不依赖于 $Y$ 本身，称之为 MAR，即：
...
old_par <- par()
png("tmp/airquality-aggr.png", width = 1200, height = 600, res = 150)
...
![](tmp/airquality-aggr.png){fig-align="center" width=100%}
:::
::::
...
old_par <- par()
png("tmp/airquality-matrixplot.png", width = 1200, height = 600, res = 150)
...
![](tmp/airquality-matrixplot.png){fig-align="center" width=100%}
:::
...
old_par <- par()
png("tmp/tao-matrixplot.png", width = 1200, height = 600, res = 150)
par(mai = c(1, 1, 0, 0), oma = c(0, 0, 0, 0))
...
![](tmp/tao-matrixplot.png){fig-align="center" width=100%}
:::
::::
...
nearZeroVar(
  x, freqCut = 95/5, uniqueCut = 10, saveMetrics = FALSE, names = FALSE, foreach = FALSE, allowParallel = TRUE)
```
...
| `names`       | `FALSE` 则返回列号；`TRUE` 则返回列名                   | `FALSE` |
...
:::: {.columns}
::: {.column width="50%"}
![](images/anomaly-example-1.jpg){fig-align="center" width=80%}
:::

::: {.column width="50%"}
<br/>

![](images/anomaly-example-2.jpg){fig-align="center" width=80%}
:::
::::
...
::: {.column width="50%"}
![](images/anomaly-comparison.png){fig-align="center" width=80%}
:::
::::

::: aside
图片来源：<https://scikit-learn.org/stable/modules/outlier_detection.html>
:::
...
::: {.center}
![](images/boxplot.svg){fig-align="center" width=60%}
:::
...
Isolation Forest，Isolation 意为孤立、隔离，是名词，Forest 是森林，合起来就是“孤立森林”了，也有叫“独异森林”，并没有统一的中文叫法，大家更习惯用其英文的名字 isolation forest，简称 iForest ^[Liu, Fei Tony, Kai Ming Ting, and Zhi-Hua Zhou. "Isolation forest." _2008 Eighth IEEE International Conference on Data Mining_. IEEE, 2008.] ^[Liu, Fei Tony, Kai Ming Ting, and Zhi-Hua Zhou. "Isolation-based anomaly detection." _ACM Transactions on Knowledge Discovery from Data (TKDD)_ 6.1 (2012): 1-39.]。
...
x <- c(rnorm(1000, 0, 0.5), rnorm(1000 * 0.05, -1.5, 1))
y <- c(rnorm(1000, 0, 0.5), rnorm(1000 * 0.05,  1.5, 1))
data <- data.frame(x, y)
...
old_par <- par()
png("tmp/isolation-forest.png", width = 800, height = 800, res = 150)
par(mai = c(1, 1, 0, 0), oma = c(0, 0, 0, 0))
...
![](tmp/isolation-forest.png){fig-align="center" width=100%}
:::
::::
...
s_i <- sampling::strata(iris_,
  stratanames = "Species", size = n, method = "srswor")
```
:::
...
:::: {.columns}
::: {.column width="50%"}
**标准化**的目的是为了让数据的均值为 0，标准差为 1，标准化还称为 Z-score，标准化的结果可以表示为：

$$
x' = \dfrac{x - \overline{X}}{S}
$$

其中，$\overline{X}$ 为 $x$ 的均值，$S$ 为 $x$ 的标准差。
...
![](images/gradient-without-normalization.svg){fig-align="center" width=100%}
:::

::: {.column width="20%"}
<br/><br/><br/><br/>

![](images/gradient-with-normalization.svg){fig-align="center" width=100%}
:::
::::
...
::: {.column width="50%"}
![](images/age.png){fig-align="center" width=70%}

![](images/binned-age.png){fig-align="center" width=70%}
:::
::::
...
- 降维
  ...
  - 等距映射算法
  - 局部线性嵌入
  - 流行学习 ^[<https://leovan.me/cn/2018/03/manifold-learning/>]：SNE，t-SNE，LargeVis
:::
...
- 表示学习
  - 文本，图像，序列...
  - Something2Vec ^[<https://gist.github.com/nzw0301/333afc00bd508501268fa7bf40cafe4e>]
:::

:::: {.columns}
::: {.column width="50%"}
![](images/linear-relationships-male-female.png){fig-align="center" width=100%}
:::

::: {.column width="50%"}
![](images/linear-relationships-verb-tense.png){fig-align="center" width=100%}
:::
::::
...
:::: {.columns}
::: {.column width="50%"}
主成分分析（Principal Components Analysis，PCA）由 Pearson ^[Pearson, Karl. "LIII. On lines and planes of closest fit to systems of points in space." _The London, Edinburgh, and Dublin Philosophical Magazine and Journal of Science_ 2.11 (1901): 559-572.] 于 1901 年提出。主成分分析可以将多个相关变量转化为少数几个不相关变量的统计分析方法。通过主成分分析 PCA 量保留原始信息的基础上，尽可能提出更少的不相关变量（主成分），可以对数据进行有效的降维。

右图是 PCA 的投影的一个表示，蓝色的点是原始的点，带箭头的橘黄色的线是投影的向量，$p_1$ 表示特征值最大的特征向量，$p_2$ 表示特征值次大的特征向量。
:::

::: {.column width="50%"}
![](images/pca.jpg){fig-align="center" width=100%}
:::
::::
...
封装方法可以通过 `mlr3fselect` 得到支持。下面我们将对用于预测患者是否患有糖尿病的 Pima 数据集进行特征选择。
...
最后，我们需要在 `terminator` 中选择一个停止准则。部分可用的 `terminator` 有：

- `clock_time`：在指定时间后停止。
- `evals`：在指定迭代次数后停止。
- `perf_reached`：在达到一个指定性能指标后停止。
- `stagnation`：在性能指标不再有提升后停止。
:::
...
:::: {.columns}
::: {.column width="50%"}
在开始特征选择前，我们需要指定一个 `fselector` 定义的算法。部分可用的 `fselector` 有：

- `random_search`：随机搜索
- `exhaustive_search`：增强搜索
- `sequential`：序列搜索
- `rfe`：递归式特征消除
...
在执行特征选择时，我们需要将 `instance` 传入到 `fselector` 的 `$optimize()` 方法中。
...
- 对于每份特征子集，`learner` 针对 `task` 利用提供的 `resampling` 进行拟合。所有执行的结果将存储在 `instance` 中。
...
:::: {.columns}
::: {.column width="50%"}
在数据分析和挖掘中，特征占据着很重要的地位。因此，我们需要对重要的特征进行监控与有效性分析，了解模型所用的特征是否存在问题，当某个特别重要的特征出现问题时，需要做好备案，防止灾难性结果。
...
::: {.column width="50%"}
![](images/data-monitor.png){fig-align="center" width=80%}
:::
::::
