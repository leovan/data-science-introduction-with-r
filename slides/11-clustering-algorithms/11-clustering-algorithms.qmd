...
::: {.center}
![](images/man-and-ed-distance.svg){fig-align="center" width=30%}
:::
...
::: {.center}
![K-means 第 1 轮](images/kmeans-example-round-1.png){fig-align="center" width=66%}
:::

## K-means

::: {.center}
![K-means 第 2 轮](images/kmeans-example-round-2.png){fig-align="center" width=66%}
:::

## K-means

::: {.center}
![K-means 第 3 轮](images/kmeans-example-round-3.png){fig-align="center" width=66%}
:::

## K-means

::: {.center}
![K-means 第 7 轮](images/kmeans-example-round-7.png){fig-align="center" width=66%}
:::
...
p <- ggplot(iris_res, aes(Petal.Length, Petal.Width)) +
  geom_point(aes(color = Cluster, shape = Cluster))
ggsave("tmp/iris-kmeans.png", width = 6, height = 4, dpi = 150)
```

![](tmp/iris-kmeans.png){fig-align="center" width=100%}
:::
::::
...
p <- factoextra::fviz_cluster(iris_kmeans, data=iris_, geom=c("point"))
ggsave("tmp/iris-kmeans-cluster.png", width = 10, height = 4, dpi = 150)
```

::: {.center}
![](tmp/iris-kmeans-cluster.png){fig-align="center" width=100%}
:::
...
::: {.column width="50%"}
![](images/agglomerative-clustering.png){fig-align="center" width=100%}
:::
::::
...
old_par <- par()
png("tmp/animals-agnes.png", width = 1000, height = 450, res = 150)
par(mai = c(1, 1, 0.5, 0), oma = c(0, 0, 0, 0))

plot(animals_agnes, which.plot = 2, main = "AGNES")

dev.off()
par(old_par)

old_par <- par()
png("tmp/animals-diana.png", width = 1000, height = 450, res = 150)
par(mai = c(1, 1, 0.5, 0), oma = c(0, 0, 0, 0))

plot(animals_diana, which.plot = 2, main = "DIANA")

dev.off()
par(old_par)
```

![](tmp/animals-agnes.png){fig-align="center" width=100%}

![](tmp/animals-diana.png){fig-align="center" width=100%}
:::
::::
...
![](images/dbscan-core-border-noise-points.svg){fig-align="center" width=70%}
...
\Procedure{DBSCAN}{$D, \epsilon, MinPts$}
    ...
    \While{$I \neq \varnothing$}
        ...
        \State 随机选取核心点 $p \in I$，并初始化队列 $Q \gets \{p\}$
        \State $U \gets U \setminus \{p\}$
        \While{$Q \neq \varnothing$}
            \State $q \gets Q$ 的队首
            \If{$\rho \geq MinPts$}
                \State $R \gets N_\epsilon \left(q\right) \cap U$, $Q \gets Q \cup R$, $U \gets U \setminus R$
            \EndIf
        \EndWhile
        \State $k \gets k+1$
...
::: {.column width="50%"}
![](images/dbscan-and-kmeans-comparision-kmeans.svg){fig-align="center" width=60%}

![](images/dbscan-and-kmeans-comparision-dbscan.svg){fig-align="center" width=60%}
:::
::::
...
p <- ggplot(points_plt, aes(x, y)) +
  geom_point(aes(color = Cluster, shape = Cluster))
ggsave("tmp/dbscan.png", width = 6, height = 5, dpi = 150)
```

![](tmp/dbscan.png){fig-align="center" width=100%}
:::
::::
